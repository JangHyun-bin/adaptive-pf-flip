...
TEST_CASE("sparse 3D tp advect reports phase boundary clamps") {
  ...
  spAdvect3D_tp(ps, g, 0.1, &stats, 2, &escapeBuffer);
  ...
  CHECK(escapeBuffer.records[0].pos.x == doctest::Approx(7.5).epsilon(1e-12));
  CHECK(escapeBuffer.records[0].volume == doctest::Approx(1.0));
  CHECK(escapeBuffer.records[1].type == 1);
  CHECK(escapeBuffer.records[1].pos.y == doctest::Approx(0.5).epsilon(1e-12));
}
