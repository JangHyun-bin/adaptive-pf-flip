...
double volumeType(const Particles3DTP& ps, unsigned char type) {
  double volume = 0.0;
  for (size_t i = 0; i < ps.size(); ++i) {
    if (ps.type[i] == type) volume += ps.volume[i];
  }
  return volume;
}
...
TEST_CASE("multires 3D surface tension smoothing reports capillary gate") {
  MRSim3DTP sim(8, 12, 8, 1.0);
  sim.dt = 0.02;
  sim.cg_iters = 20;
  sim.surface_tension = true;
...
TEST_CASE("multires 3D secondary lifecycle can reabsorb droplets into primary particles") {
  MRSim3DTP sim(8, 12, 8, 1.0);
  sim.dt = 0.1;
  sim.gravity = 0.0;
  sim.cg_iters = 20;
