...
Generated UTC: `2026-06-19T09:37:33+00:00`
Status: `warning`
...
- CSV profile: `build\shots\s198_water_mesh_quality_diagnostics\water_mesh_quality_profile.csv`
- JSON summary: `build\shots\s198_water_mesh_quality_diagnostics\water_mesh_quality_summary.json`
...
- The warning is caused by component fragmentation, not broken OBJ topology:
  degenerate, boundary, and non-manifold edge ratios are all zero across the
  diagnostic run.
- Worst frames are early source frames `0` through `6`, where the mesh splits
  into two closed components and the largest component ratio falls to roughly
  `0.77`.

## Next

Start with reconstruction component/island handling. The next pass should add
component metadata and an optional small-island pruning or labeling stage before
trying stronger normals or renderer smoothing.
