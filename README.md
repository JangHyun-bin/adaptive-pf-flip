...
| **S80** | **Water reflection continuity** - `dam_break_water_reflection_continuity` adds longer camera-stable water reflection ribbons over S79 glints to reduce the flat-slab read of the main water body. | done |
| **S81** | **Water highlight temporal QA** - `dam_break_water_highlight_temporal_review` gates frame-to-frame luminance and highlight-mask deltas so glint/reflection layers can be checked for flicker. | done |
| **S82** | **Water highlight temporal diff sheet** - `dam_break_water_highlight_temporal_diff_sheet` emits frame-difference review artifacts so highlight motion can be inspected visually. | done |
| **S83** | **Water highlight motion tuning** - `dam_break_water_highlight_motion_tuned` increases glint/reflection drift and coverage while preserving temporal highlight QA. | done |
| **S84** | **Water impact ripple cues** - `dam_break_water_impact_ripple_cues` adds foam/spray-driven impact-region ripple arcs so the splash contact area reads as broken water surface. | done |
| **S85** | **Water impact ripple tuning** - `dam_break_water_impact_ripple_tuned` lowers ripple density and softens material strength so contact breakup remains readable without overpowering foam/spray. | done |
| **S86** | **Water impact ripple material fade** - `dam_break_water_impact_ripple_fade` adds UV edge-falloff material to impact ripple arcs so contact breakup blends into the water surface. | done |
| **S87** | **Water impact ripple focus QA** - `dam_break_water_impact_ripple_focus_qa` adds a focused contact-region review sheet and gate for ripple readability. | done |
| **S88** | **Water impact ripple focus comparison** - focus review sheets can now be compared across nearby cinematic gates without replacing the full-frame contact sheet. | done |
| **S89** | **Contact focus camera tuning** - `dam_break_contact_focus_camera_tuned` narrows the contact camera and focus crop while preserving visual, temporal, camera, and secondary framing gates. | done |
| **S90** | **Ripple placement focus tuning** - `dam_break_ripple_placement_focus_tuned` widens and slightly strengthens contact-region impact ripples while preserving all cinematic gates. | done |
| **S91** | **Ripple readability diagnostics** - `dam_break_ripple_readability_diagnostics` adds edge/highlight diagnostic sheets for contact-region ripple readability review. | done |
| **S92** | **Ripple diagnostic comparison** - ripple readability diagnostic sheets can now be compared side by side across nearby cinematic gates. | done |
| **S93** | **Contact foam/ripple integration** - `dam_break_contact_foam_ripple_integrated` softens and narrows contact foam so surface breakup reads less like separate overlay layers. | done |
| **S94** | **Water surface breakup/noise tuning** - `dam_break_water_surface_breakup_noise_tuned` increases water surface detail strength/depth for stronger contact-region breakup while preserving diagnostic and temporal gates. | done |
| **S95** | **Spray/foam depth layering** - `dam_break_spray_foam_depth_layered` reduces spray/foam radius, soft-pass brightness, and foam streak intensity so secondaries sit more naturally in the contact volume. | done |
| **S96** | **Secondary depth review metric** - `dam_break_secondary_depth_reviewed` adds a projected spray/foam contact-volume diagnostic sheet and gate for crop count, depth span, and normalized depth span. | done |
| **S97** | **Secondary volume-depth material tuning** - `dam_break_secondary_volume_depth_tuned` lowers spray/foam emission and streak brightness while preserving secondary depth, visual, temporal, and ripple gates. | done |
...
| **S102** | **Water volume scattering pass** - `dam_break_water_volume_scattering` adds opt-in internal attenuation sheets so the main water body gains subtle volume/scattering cues while preserving the S100 diagnostic gates. | done |
| **S103** | **Secondary render integration review** - `dam_break_secondary_render_integration_review` extends the S102 water volume baseline with bubble-inclusive secondary depth review and comparison. | done |
| **S104** | **Large-grid cinematic benchmark** - `dam_break_large_grid_cinematic_benchmark` runs the S103 render/review stack on a `32 x 40 x 26` grid with larger-grid camera and secondary acceptance gates. | done |
...
| **S113** | **Warm-cache stage summary** - `tools/summarize_shot_commands.py` reports command timings and reuse flags from a cinematic `shot_summary.json`. | done |
...
| **S147** | **S145 public gallery visual triage** - reviews the S145 public gallery and selects S148 foreground water thickness/refraction as the next visible look-dev pass. | done |
