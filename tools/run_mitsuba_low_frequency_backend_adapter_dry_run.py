#!/usr/bin/env python
"""Dry-run low-frequency backend scene descriptors."""
...
def require_pillow():
    if Image is None:
        raise SystemExit("Pillow is required to dry-run low-frequency backend descriptors")
...
def summary_metadata_entry(summary_path, assets_dir, root):
    dest = os.path.join(assets_dir, "backend_adapter_dry_run_summary.json")
    return {
        "label": "Backend Adapter Dry Run Summary",
        "repo_path": posix_rel(dest, root),
        "href": "assets/backend_adapter_dry_run_summary.json",
        "source_repo_path": posix_rel(summary_path, root),
    }


def load_scene(frame, root):
    scene_info = frame.get("scene_descriptor") or {}
    scene_path = require_file(resolve_path(scene_info.get("repo_path"), root), "backend scene descriptor")
    scene = read_json(scene_path)
    return scene_path, scene


def run_scene(frame, root, strip_dir):
    scene_path, scene = load_scene(frame, root)
    ...
    if missing:
        return {"status": "failed", "frame": scene.get("frame"), "missing": missing, "scene_repo_path": posix_rel(scene_path, root)}
    ...
    if any(image.size != base.size for image in (positive, negative, reference)):
        return {"status": "failed", "frame": scene.get("frame"), "dimension_mismatch": True, "scene_repo_path": posix_rel(scene_path, root)}
    gain = float(((scene.get("runtime_contract") or {}).get("parameters") or {}).get("texture_gain", 1.0))
    ...
    strip_path = os.path.join(strip_dir, f"frame_{scene.get('job_index', 0):04d}_backend_dry_run.png")
    labeled_strip(
...
def html_page(title, summary, assets, metadata_files):
    shot = next((item for item in assets if item.get("label") == "Backend Dry Run GIF"), None)
    strips = [item for item in assets if item.get("label", "").startswith("Backend Dry Run Strip")]
    ...
    hero = f'<section class="hero"><img src="{shot["href"]}" alt="Backend dry-run GIF"></section>' if shot else ""
    frame_html = "\n".join(
...
def markdown_report(summary, summary_path, root):
    ...
    lines = [
        ...
        "## Frame Results",
        "",
        "| Job | Frame | Output | Status | Max Diff | Scene | Output |",
        "| ---: | ---: | ---: | --- | ---: | --- | --- |",
    ]
    for frame in summary.get("frames") or []:
        lines.append(
            f"| {frame.get('job_index')} | {frame.get('frame')} | {frame.get('output_frame')} | `{frame.get('status')}` | "
            f"{frame.get('max_abs_diff')} | `{frame.get('scene_repo_path')}` | `{frame.get('output_image_repo_path')}` |"
        )
    lines.extend(["", "## Next", "", summary.get("next") or "Use this descriptor-layer proof before swapping in a real backend process.", ""])
    return "\n".join(lines)
...
def run_adapter(args):
    ...
    results = [run_scene(frame, root, strip_dir) for frame in manifest.get("frames") or []]
    ...
    strip_gif_path = os.path.join(assets_dir, "backend_dry_run_strips.gif")
    if output_paths:
    ...
    if os.path.isfile(gif_path):
        assets.append(copy_asset(gif_path, assets_dir, "shot.gif", "Backend Dry Run GIF", root))
    if os.path.isfile(strip_gif_path):
        assets.append(copy_asset(strip_gif_path, assets_dir, "backend_dry_run_strips.gif", "Backend Dry Run Strip GIF", root))
    ...
    for out_index, frame_index in enumerate(key_indices):
        assets.append(copy_asset(passed[frame_index]["output_image_repo_path"], assets_dir, f"keyframe_{out_index:02d}.png", f"Backend Dry Run Keyframe {out_index + 1}", root))
        assets.append(copy_asset(passed[frame_index]["strip_repo_path"], assets_dir, f"backend_dry_run_strip_{out_index:02d}.png", f"Backend Dry Run Strip {out_index + 1}", root))
    summary_path = os.path.abspath(args.summary)
    checks = {
        "frames": len(results),
        "passed_frames": len(passed),
        "failed_frames": len(failed),
        "missing_frames": sum(1 for item in results if item.get("missing")),
    ...
    summary = {
        "schema": "lsfs_mitsuba_low_frequency_backend_adapter_dry_run",
        "version": 1,
        "generated_utc": datetime.now(timezone.utc).isoformat(),
        "title": args.title,
        "status": status,
        ...
        "settings": {
            ...
            "backend_kind": (manifest.get("backend") or {}).get("kind"),
        },
        "checks": checks,
        "frames": results,
        "gallery": {},
    ...
    write_json(os.path.join(gallery_dir, "gallery_manifest.json"), {
        "schema": "lsfs_mitsuba_low_frequency_backend_adapter_dry_run_gallery",
        "version": 1,
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Dry-run low-frequency backend scene descriptors")
    ...
    parser.add_argument("--title", default="S503 Mitsuba Low Frequency Backend Adapter Dry Run")
    parser.add_argument("--next", default="Use this descriptor-layer proof before replacing the skeleton with a real backend process.")
