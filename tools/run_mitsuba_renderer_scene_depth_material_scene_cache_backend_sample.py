#!/usr/bin/env python
"""Run scene-cache-derived depth/material controls through the backend sample."""
...
from run_mitsuba_renderer_scene_depth_material_backend_sample import (
    SCENE_SCHEMA,
    STAGE,
    ensure_dir,
    file_entry,
...
from run_mitsuba_renderer_scene_depth_material_sweep import load_inputs


def target_by_frame(target):
    return {int(frame.get("frame") or 0): frame for frame in target.get("frames") or []}


def path_file_ref(path, root, role):
    if not path or not os.path.isfile(path):
        return {"role": role, "status": "missing", "repo_path": posix_rel(path, root) if path else None}
...
def scene_descriptor(item, target_frame, job_index, root, out_dir, backend_script, base_strength):
    frame_id = int(item.get("frame") or 0)
    frame_name = f"frame_{frame_id:04d}"
    scene_path = os.path.abspath(os.path.join(out_dir, "scenes", f"{frame_name}_scene_cache_backend_scene.json"))
    ...
    effective_strength = base_strength * (0.30 + 0.45 * float(item.get("water_z_factor") or 0.0) + 0.25 * float(item.get("water_y_factor") or 0.0))
    ...
    descriptor = {
        ...
        "backend": {
            ...
            "adapter_mode": "scene_cache_direct_external_backend_sample",
        },
        "controls": {
            "base_strength": base_strength,
            "effective_strength": effective_strength,
            "water_y_factor": item.get("water_y_factor"),
            "water_z_factor": item.get("water_z_factor"),
            "formula": "base_strength * (0.30 + 0.45 * water_z_factor + 0.25 * water_y_factor)",
        },
        "inputs": {
            "source_composite": path_file_ref(item.get("composite_path"), root, "source_composite"),
            "magnitude_mask": path_file_ref(item.get("magnitude_path"), root, "magnitude_mask"),
            "target_preview": target_file_ref(refs.get("target_preview"), root, "target_preview"),
        },
        "outputs": {
            "image": {"repo_path": posix_rel(output_image, root), "format": "png", "semantics": "scene-cache backend tonemap output"},
            "metadata": {"repo_path": posix_rel(output_metadata, root), "format": "json", "semantics": "scene-cache backend metadata"},
            "validation": {"repo_path": posix_rel(output_validation, root), "format": "json", "semantics": "scene-cache backend validation"},
        },
...
def html_page(summary):
    ...
    gif = next((item for item in assets if item.get("label") == "Scene-Cache Backend Sample GIF"), None)
    strips = [item for item in assets if item.get("label", "").startswith("Scene-Cache Backend Sample Strip")]
    ...
    hero = f'<section class="hero"><img src="{gif["href"]}" alt="scene-cache backend sample gif"></section>' if gif else ""
    figures = "\n".join(
...
def markdown_report(summary, summary_path, root):
    ...
    lines = [
        ...
        "## Inputs",
        "",
        f"- Handoff manifest: `{summary['inputs']['handoff_manifest']}`",
...
def run(args):
    ...
    handoff_path = require_file(resolve_path(args.handoff_manifest, root), "renderer scene-cache handoff")
    render_data_path = require_file(resolve_path(args.render_data_summary, root), "renderer scene render-data summary")
    ...
    target_frames = target_by_frame(target)
    base_strength = float((target.get("selected") or {}).get("base_strength") or args.base_strength)
    loaded = load_inputs(handoff_path, render_data_path, root)
    ...
    chosen = selected_frames(loaded["frames"], args.frames)
    results = []
    for job_index, item in enumerate(chosen):
        target_frame = target_frames.get(int(item.get("frame") or 0), {})
        scene_path, descriptor = scene_descriptor(item, target_frame, job_index, root, out_dir, backend_script, base_strength)
    ...
    gif_path = os.path.join(out_dir, "scene_cache_backend_sample.gif")
    strip_gif_path = os.path.join(out_dir, "scene_cache_backend_sample_strips.gif")
    if output_paths:
    ...
    if os.path.isfile(gif_path):
        assets.append(copy_asset(gif_path, assets_dir, "scene_cache_backend_sample.gif", "Scene-Cache Backend Sample GIF", root))
    if os.path.isfile(strip_gif_path):
        assets.append(copy_asset(strip_gif_path, assets_dir, "scene_cache_backend_sample_strips.gif", "Scene-Cache Backend Sample Strip GIF", root))
    ...
    for out_index, frame_index in enumerate(key_indices):
        assets.append(copy_asset(passed[frame_index]["strip_repo_path"], assets_dir, f"scene_cache_backend_sample_strip_{out_index:02d}.png", f"Scene-Cache Backend Sample Strip {out_index + 1}", root))
    ...
    checks = {
        ...
        "input_missing_references": len(loaded.get("missing") or []),
    ...
    status = "passed" if (
        ...
        and checks["input_missing_references"] == 0
    ...
    metadata_files = [
        ...
        copy_asset(render_data_path, assets_dir, "render_data_summary.json", "Renderer Scene Render-Data Summary", root),
    ...
    summary = {
        "schema": "lsfs_mitsuba_renderer_scene_depth_material_scene_cache_backend_sample",
        "version": 1,
        "generated_utc": datetime.now(timezone.utc).isoformat(),
        "title": args.title,
        "status": status,
        "backend": {
            ...
            "mode": "scene_cache_direct_external_backend_sample",
        },
        "inputs": {
    ...
    metadata_files.insert(0, copy_asset(summary_path, assets_dir, "scene_cache_backend_sample_summary.json", "Scene-Cache Backend Sample Summary", root))
    ...
    write_json(os.path.join(gallery_dir, "gallery_manifest.json"), {
        "schema": "lsfs_mitsuba_renderer_scene_depth_material_scene_cache_backend_sample_gallery",
        "version": 1,
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="Run scene-cache depth/material controls through a backend executable sample")
    ...
    parser.add_argument("--base-strength", type=float, default=1.0)
    parser.add_argument("--frames", type=int, default=8)
    parser.add_argument("--fps", type=float, default=12.0)
    parser.add_argument("--keyframes", type=int, default=4)
    parser.add_argument("--frame-timeout", type=float, default=60.0)
    parser.add_argument("--title", default="S590 Mitsuba Renderer Scene Depth Material Scene-Cache Backend Sample")
    parser.add_argument(
        "--next",
        default="Use this scene-cache direct backend sample as the input contract for a renderer-native material or tonemap implementation.",
    )
    args = parser.parse_args(argv)
    if args.base_strength <= 0.0 or args.base_strength > 1.0:
        parser.error("base-strength must be in (0, 1.0]")
    if args.frames <= 0:
